...
So if either stream is piped or redirected, alarm does not auto-run unless explicitly requested with `--sound`.

When `--sound` is provided, alarm playback is still attempted on completion in `--quiet` and non-TTY modes.

On macOS, default sleep inhibition is attempted only when both `stdout` and `stderr` are TTYs. In that interactive mode, timer uses `caffeinate -i -d -w <pid>` to also keep the display awake. With `--caffeinate`, timer also attempts sleep inhibition in non-TTY/piped runs (best effort), but uses `caffeinate -i -w <pid>` (no `-d`).
